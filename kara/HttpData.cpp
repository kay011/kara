...
const int DEFAULT_EXPIRED_TIME = 2000;  // ms  默认定时时间
const int DEFAULT_KEEP_ALIVE_TIME = 5 * 60 * 1000; // ms 默认TCP keepalive时间
...
HttpData::HttpData(EventLoop *loop, int connfd)
    ...
    fd_(connfd),
    error_(false),
    connectionState_(H_CONNECTIONED),
    method_(METHOD_GET),
...
void HttpData::handleRead(){
    ...
    do{
        ...
        if(read_num < 0){
            ...
            // 有请求出现，但是读不到数据，可能是Request
            // Aborted， 或者来自网络的数据没有达到等原因
            // 最有可能是对端已经关闭了，同意按照对端已经关闭处理
...
void HttpData::handleConn(){
    ...
    seperateTimer();
    __uint32_t &events_ = channel_ -> getEvents();
    if(!error_ && connectionState_ == H_CONNECTIONED){
        if(events_ != 0){
...
void HttpData::handleClose(){
    ...
    shared_ptr<HttpData> guard(shared_from_this());  // 啥意思
